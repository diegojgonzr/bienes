...
class ModelForm
{
    ...
    public static function create($model, $action = NULL)
    {
            ...
            switch ($tipo) {
                ...
                case 'year': case 'day': case 'int unsigned': // Números
                    ...
                    if (strripos($field, '_id', -3)) {
                        ...
                        echo "<input id=\"$formId\" type=\"number\" class=\"form-control\" name=\"$formName\" value=\"{$model->$field}\">" . PHP_EOL;
                        break;
                    }

                case 'date': // Usar el js de datetime
                    echo "<input id=\"$formId\" type=\"date\" class=\"form-control\" name=\"$formName\" value=\"{$model->$field}\">" . PHP_EOL;
                    break;
                case 'datetime': case 'timestamp':
                    echo "<input id=\"$formId\" type=\"datetime\" class=\"form-control\" name=\"$formName\" value=\"{$model->$field}\">" . PHP_EOL;
                ...
                case 'enum': case 'set': case 'bool':
                    ...
                    echo "<input id=\"$formId\" class=\"select\" class=\"form-control\" name=\"$formName\" type=\"text\" value=\"{$model->$field}\">" . PHP_EOL;
                    break;

                case 'text': case 'mediumtext': case 'longtext':
                case 'blob': case 'mediumblob': case 'longblob': // Usar textarea
                    echo "<textarea id=\"$formId\" class=\"form-control\" name=\"$formName\">{$model->$field}</textarea>" . PHP_EOL;
                    break;
        ...
        echo '<input type="submit" class="btn btn-primary" value="Enviar" />' . PHP_EOL;
        echo '</form>' . PHP_EOL;
    }
