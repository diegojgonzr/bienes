...
class DwMenu {
    ...
    public static function desktop() {
        ...
        if(self::$_main) {
            ...
            $html.= '<li class="header">MENU PRINCIPAL</li>'.PHP_EOL;
            foreach(self::$_main as $main) {         
                $active = ($main->url==$route) ? 'active' : null;
                if(self::$_entorno==Menu::BACKEND) {
                    $html.= '<li class="'.$active.' treeview"><a href="'.$main->url.'"><i class="fa '.$main->icono.'"></i><span>'.$main->menu.'</span><span class="pull-right-container">
                        <i class="fa fa-angle-left pull-right"></i>
                        </span></a>';
                        $menu = new Menu();
                    $submenu = $menu->getListadoSubmenuPorPerfil(self::$_entorno, self::$_perfil, $main->id);
                    if($submenu) {
                        $html.= '<ul class="treeview-menu">';
                        foreach($submenu as $tmp) {
                            $html.= '<li>'.DwHtml::link($tmp->url, $tmp->menu, null, "fa fa-circle-o").'</li>'.PHP_EOL;
                        }
                        $html.= '</ul>'.PHP_EOL;
                    } 
                    $html.='</li>'.PHP_EOL;
                    ...
                    if(!array_key_exists($main->menu, self::$_items)) {
                        ...
                        foreach(self::$_items[$main->menu] as $item) {                        
                            ...
                            $html.= '<li class="'.$active.'">'.DwHtml::link($item->url, $item->menu, NULL, $item->icono, APP_AJAX).'</li>';
                        }                        
                        $html.= '</ul>';
                        $html.= '</li>';
                    } else {
            ...
            foreach(self::$_main as $main) {
                ...
                if(array_key_exists($main->menu, self::$_items)) {
                    ...
                    foreach(self::$_items[$main->menu] as $item) { 
                        ...
                        $active = ($item->url==$route) ? 'active' : null;                        
                        $html.= '<li class="'.$active.'">'.DwHtml::link($item->url, $item->menu, NULL, $item->icono, TRUE).'</li>';
                    }
                    $html.= '</ul>';
                }
                $html.= '</li>'.PHP_EOL;
    ...
    public function getSubMenu(int $id=null){

        $submenu = $item->getListadoSubmenuPorPerfil(self::$_entorno, self::$_perfil, $id);
        if($submenu) {
            $html.= '<ul class="treeview-menu">';
            foreach($submenu as $tmp) {
                $html.= '<li>'.DwHtml::link($tmp->url, $tmp->menu, null, $tmp->icono).'</li>'.PHP_EOL;
